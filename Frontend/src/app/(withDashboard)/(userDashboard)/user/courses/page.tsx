...
const UserCoursesPage = async () => {
    ...
    if (user && (user.role === 'student' || user.role === 'admin') && typeof user.email === 'string') {
        courses = await getUserCourses(user.email);
    }
    return (
        <div className="max-w-2xl mx-auto mt-10 p-6 bg-white rounded shadow">
            <h2 className="text-2xl font-bold mb-4 text-center">My Enrolled Courses</h2>
            {courses.length > 0 ? (
                ...
                <div className="text-center text-gray-500">You have not enrolled in any courses yet.</div>
            )}
        </div>
    );
};
